...
class HubRedisStream:
    ...
    async def read_pending_events(self, count: int) -> list[tuple[str, dict[str, str]]]:
        return await self._read_events(stream_id="0", count=count, block_ms=None)

    async def read_new_events(self, count: int) -> list[tuple[str, dict[str, str]]]:
        return await self._read_events(
            stream_id=">",
            count=count,
            block_ms=self._settings.events_block_ms,
        )

    async def ack_event(self, message_id: str) -> None:
        await self._client.xack(
            self._settings.events_stream,
            self._settings.events_group,
            message_id,
        )
    ...
    async def _read_events(
        ...
        rows = await self._client.xreadgroup(
            groupname=self._settings.events_group,
            consumername=self._settings.events_consumer,
            streams={self._settings.events_stream: stream_id},
            count=count,
            block=block_ms,
        )
        return _flatten_stream_entries(rows)
